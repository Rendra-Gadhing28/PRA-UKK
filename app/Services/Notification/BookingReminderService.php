    ...
    public function processReminders(): void
    {
        ...
        foreach ($bookings as $booking) {
            ...
            if ($diffInHours <= 24 && $diffInHours > 1 && ! $booking->is_h24_reminded) {
                try {
                    Mail::to($targetEmail)->send(new BookingReminderMail($booking, 'H-1 Hari'));
            ...
            if ($diffInHours <= 1 && $diffInMinutes > 30 && ! $booking->is_h1_reminded) {
                try {
                    Mail::to($targetEmail)->send(new BookingReminderMail($booking, 'H-1 Jam'));
            ...
            if ($diffInMinutes <= 30 && ! $booking->is_m30_reminded) {
                try {
                    Mail::to($targetEmail)->send(new BookingReminderMail($booking, 'H-30 Menit'));
