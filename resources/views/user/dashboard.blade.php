<x-app-layout>
    ...
    <div x-data="dashboardPage()" class="relative z-10 min-h-screen pt-28 pb-24">
            ...
            <div class="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
                ...
                <div class="xl:col-span-8 space-y-4">
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        ...
                        <div class="relative w-full md:w-64">
                            <input x-model="searchQuery" class="w-full pl-10 pr-4 py-2 rounded-full border-border-subtle bg-surface focus:ring-primary focus:border-primary text-body-md font-body-md text-on-surface transition-all" placeholder="Cari treatment..." type="text"/>
                    ...
                    <div class="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
                        <button @click="selectedCategory = 'all'" :class="selectedCategory === 'all' ? 'bg-primary text-white' : 'border border-border-subtle bg-surface-light text-on-surface-variant hover:bg-surface-container'" class="px-4 py-2 rounded-full font-label-md text-label-md whitespace-nowrap transition-colors">Semua</button>
                        <button @click="selectedCategory = 'facial'" :class="selectedCategory === 'facial' ? 'bg-primary text-white' : 'border border-border-subtle bg-surface-light text-on-surface-variant hover:bg-surface-container'" class="px-4 py-2 rounded-full font-label-md text-label-md whitespace-nowrap transition-colors">Facial</button>
                        <button @click="selectedCategory = 'hair'" :class="selectedCategory === 'hair' ? 'bg-primary text-white' : 'border border-border-subtle bg-surface-light text-on-surface-variant hover:bg-surface-container'" class="px-4 py-2 rounded-full font-label-md text-label-md whitespace-nowrap transition-colors">Hair</button>
                        <button @click="selectedCategory = 'nails'" :class="selectedCategory === 'nails' ? 'bg-primary text-white' : 'border border-border-subtle bg-surface-light text-on-surface-variant hover:bg-surface-container'" class="px-4 py-2 rounded-full font-label-md text-label-md whitespace-nowrap transition-colors">Nails</button>
                        <button @click="selectedCategory = 'massage'" :class="selectedCategory === 'massage' ? 'bg-primary text-white' : 'border border-border-subtle bg-surface-light text-on-surface-variant hover:bg-surface-container'" class="px-4 py-2 rounded-full font-label-md text-label-md whitespace-nowrap transition-colors">Massage</button>
                    </div>

                    {{-- Treatment Grid (Only Top 3 Rated) --}}
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        @forelse($topTreatments as $treatment)
                            <div x-show="matchesFilter('{{ strtolower($treatment->name) }}', '{{ strtolower($treatment->description) }}', '{{ strtolower($treatment->category->name ?? '') }}')"
                                ...
                                <div class="h-36 overflow-hidden relative">
                                    <img alt="{{ $treatment->name }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" src="{{ $treatment->image_url }}"/>
                                ...
                                <div class="p-4 flex flex-col flex-1">
                                    <div class="flex justify-between items-start mb-2 gap-2">
                                        <h3 class="font-headline-sm text-base text-text-heading line-clamp-1 font-semibold">{{ $treatment->name }}</h3>
                                        <div class="flex items-center gap-1 text-primary shrink-0 bg-primary/10 px-2 py-1 rounded-md">
                                            ...
                                            <span class="font-label-md text-label-md font-bold">{{ number_format($treatment->rating ?? 4.9, 1) }}</span>
                                        </div>
                                    </div>
                                    <div class="flex items-center gap-3 text-on-surface-variant font-body-sm text-xs mb-2">
                                        <span class="flex items-center gap-1"><span class="material-symbols-outlined text-sm">schedule</span> {{ $treatment->duration_minutes }} min</span>
                                        <span class="flex items-center gap-1 font-semibold text-primary"><span class="material-symbols-outlined text-sm">payments</span> Rp {{ number_format($treatment->price, 0, ',', '.') }}</span>
                                    </div>
                                    <p class="font-body-sm text-xs text-on-surface-variant mb-4 line-clamp-2">{{ $treatment->description }}</p>
                                    <a href="{{ route('user.bookings.create', ['treatment_id' => $treatment->id]) }}" class="mt-auto w-full py-2 bg-primary hover:bg-primary-container text-white rounded-full font-button text-xs text-center shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2 group-hover:scale-[1.02]">
                                        <span>Book Now</span>
                ...
                <div class="xl:col-span-4 space-y-4">
                    <div class="glass-card rounded-2xl p-4 bg-gradient-to-b from-surface-light via-surface-container-low to-surface-light border-2 border-primary/20 shadow-xl relative overflow-hidden">
                        ...
                        {{--
                            Dot Progress Tracker — supaya progres 7 hari langsung
                            kelihatan sekilas tanpa perlu scroll baca satu-satu.
                        --}}
                        <div class="relative flex items-center justify-between mb-4 px-1">
                            <div class="absolute left-1 right-1 top-1/2 -translate-y-1/2 h-1 bg-surface-container-highest rounded-full"></div>
                            <div class="absolute left-1 top-1/2 -translate-y-1/2 h-1 bg-gradient-to-r from-amber-400 to-primary rounded-full transition-all duration-500"
                                 :style="`width: calc(${(currentStreak / missions.length) * 100}% - 4px)`"></div>

                            <template x-for="(mission, index) in missions" :key="'dot-'+index">
                                <div class="relative z-10 w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 bg-white transition-all shrink-0"
                                     :class="{
                                         'bg-primary text-white border-primary': mission.status === 'completed',
                                         'bg-white text-primary border-primary ring-4 ring-primary/20 animate-pulse': mission.status === 'active',
                        ...
                        {{--
                            Kartu "Misi Hari Ini" — gradient border emas-pink supaya
                            terasa premium & jadi satu-satunya fokus utama user.
                        --}}
                        <div class="rounded-2xl p-[2px] bg-gradient-to-r from-amber-400 via-primary to-secondary mb-4 shadow-md">
                            <div class="bg-white rounded-2xl p-4">
                                <div class="flex items-center justify-between mb-2">
                                    <span class="text-xs font-bold uppercase tracking-wider text-primary">✨ Misi Hari Ini</span>
                                    <span class="text-xs font-bold px-2 py-1 rounded-full bg-amber-100 text-amber-700" x-text="activeMission.reward"></span>
                                </div>
                        ...
                        <button @click="showAllMissions = !showAllMissions" type="button" class="w-full flex items-center justify-center gap-1 text-xs font-semibold text-primary mb-4 hover:underline">
                                ...
                                <div class="relative z-10 flex items-center gap-2 transition-all duration-300 transform"
                                    ...
                                    <div class="flex-1 p-2 rounded-xl border transition-all"
                                         :class="{
                                             ...
                                             'bg-white border-2 border-primary shadow-md shadow-primary/10': mission.status === 'active',
                                        ...
                                        <div class="flex justify-between items-center mb-1">
                                            ...
                                            <span class="text-xs font-bold px-2 py-1 rounded-full"
                                                  :class="mission.status === 'active' ? 'bg-primary text-white' : 'bg-primary/10 text-primary'"
                        ...
                        {{--
                            Claim CTA — gradient emas-pink + efek shimmer supaya
                            terasa "premium" dan jelas ini tombol utama.
                        --}}
                        <button @click="claimTodayReward()"
                                ...
                                :class="hasClaimedToday
                                    ...
                                    : 'text-white shimmer-btn bg-[linear-gradient(110deg,#f45472,45%,#fbbf24,55%,#f45472)] hover:shadow-2xl'">
        ...
        <div x-show="showQrModal" x-cloak
            ...
            <div @click.away="showQrModal = false" class="bg-surface-light rounded-2xl max-w-md w-full p-6 shadow-2xl border border-border-subtle relative text-center space-y-4">
                <button @click="showQrModal = false" class="absolute top-4 right-4 text-on-surface-variant hover:text-primary transition-colors">
    ...
    <style>
        ...
        /* Shimmer emas-pink untuk tombol klaim hadiah — kesan "premium/luxury" */
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('dashboardPage', () => ({
                ...
                missions: [
                    { dayTitle: 'Day 1: Muka Bantal Eradication', reward: '+5 pts', funnyText: 'Membasmi jejak bantal di pipi saat bangun jam 11 siang.', status: 'completed' },
                    { dayTitle: 'Day 2: Bebas Kusam Anti-Galau', reward: '+10 pts', funnyText: 'Eksfoliasi pikiran negatif & mantan yang bikin kusam.', status: 'completed' },
                    { dayTitle: 'Day 3: Glow Up Striking Back (Hari Ini)', reward: '+15 pts', funnyText: 'Aura cantik memancar 1000 watt. Siap bikin pangling!', status: 'active' },
                    { dayTitle: 'Day 4: Keindahan Hakiki Level 4', reward: '+20 pts', funnyText: 'Kuku manja & kulit selembut sutra menanti.', status: 'locked' },
                    { dayTitle: 'Day 5: Ratu Skincare Masuk Vibe', reward: '+25 pts', funnyText: 'Darah sultan mengalir, siap perawatan ala putri.', status: 'locked' },
                    { dayTitle: 'Day 6: Gratis Ongkir Auto-Sultan', reward: 'Gratis Ongkir', funnyText: 'Penghargaan khusus untuk ratu yang malas bayar ongkir!', status: 'locked' },
                    { dayTitle: 'Day 7: Mahkota Glowing 5% Off', reward: 'Diskon 5% + 100 Pts', funnyText: 'Puncak komedi & kecantikan! Diskon Ratu Beauty milikmu!', status: 'locked' }
                ],

                // Misi yang statusnya 'active' — jadi satu-satunya fokus di kartu spotlight.
                get activeMission() {
                    return this.missions.find(m => m.status === 'active') || this.missions[0];
                },

                init() {
                    const claimed = localStorage.getItem('yalia_claimed_day3');
                    if (claimed) {
                        this.hasClaimedToday = true;
                        this.missions[2].status = 'completed';
                        this.missions[2].dayTitle = 'Day 3: Glow Up Striking Back';
                        if (this.missions[3]) this.missions[3].status = 'active';
                    }
                },
                ...
                claimTodayReward() {
                    ...
                    this.missions[2].status = 'completed';
                    this.missions[2].dayTitle = 'Day 3: Glow Up Striking Back (Selesai)';
                    if (this.missions[3]) {
                        this.missions[3].status = 'active';
                    }

                    localStorage.setItem('yalia_claimed_day3', 'true');
