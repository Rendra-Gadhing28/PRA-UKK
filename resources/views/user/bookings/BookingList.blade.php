...
@else
    <div class="space-y-6">
        @foreach ($bookings as $booking)
            @php 
                $statusObj = is_string($booking->status) ? (\App\Enums\BookingStatus::tryFrom($booking->status)) : $booking->status;
                $statusVal = is_object($statusObj) && isset($statusObj->value) ? $statusObj->value : (string)$booking->status;
                ...
                $badgeClasses = is_object($statusObj) && method_exists($statusObj, 'badgeClasses') ? $statusObj->badgeClasses() : 'bg-gray-100 text-gray-800';
                ...
                $totalDuration = $booking->treatments->sum('duration_minutes') ?: ($firstTreatment?->duration_minutes ?? 0);
                $timeStartFormatted = $booking->time_start ? \Illuminate\Support\Carbon::parse($booking->time_start)->format('h:i A') : '-';
                $timeEndFormatted = $booking->time_end ? \Illuminate\Support\Carbon::parse($booking->time_end)->format('h:i A') : '-';
            @endphp
            <div class="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-border-subtle flex flex-col lg:flex-row gap-8 items-center group transition-all hover:shadow-md {{ $statusVal === 'pending' ? 'opacity-90' : '' }}">

                <div class="relative w-full lg:w-48 h-48 rounded-2xl overflow-hidden shrink-0">
                    <img
                        class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                        src="{{ $firstTreatment?->image ? asset('storage/'.$firstTreatment->image) : 'https://ui-avatars.com/api/?name='.urlencode($treatmentNames).'&background=ffe8ed&color=b01f44&size=256' }}"
                        alt="{{ $treatmentNames }}"
                        loading="lazy"
                    >
                </div>
                ...
                <div class="flex-1 w-full space-y-4">
                    <div class="flex flex-wrap justify-between items-start gap-4">
                        <div>
                            <span class="px-3 py-1 rounded-full text-label-md font-label-md {{ $badgeClasses }}">
                                {{ $badgeLabel }}
                            </span>
                            <h3 class="text-headline-sm font-headline-sm mt-2">{{ $treatmentNames }}</h3>
                        ...
                        <div class="text-right">
                            ...
                            <p class="text-body-sm font-body-sm text-on-surface-variant">{{ $totalDuration }} Minutes</p>
                        </div>
                    </div>
                ...
                <div class="flex flex-col gap-3 w-full lg:w-48">
                    ...
                    @if ($statusVal === 'pending' && $booking->payment_status !== 'paid')
