        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="font-bold text-2xl text-gray-900 tracking-tight flex items-center gap-2 font-headline">
                    Edit Beautician — {{ $beautician->name }}
                </h2>
                <p class="text-xs text-gray-500 mt-0.5">Perbarui profil, nomor telepon, foto, atau status penugasan</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-3xl p-8 shadow-sm border border-rose-100">
                <form method="POST" action="{{ route('admin.beauticians.update', $beautician->id) }}" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="name" value="{{ old('name', $beautician->name) }}" required 
                        ...
                        <div>
                            ...
                            <input type="text" name="phone" value="{{ old('phone', $beautician->phone) }}" 
                                   class="w-full px-4 py-3 text-sm rounded-2xl border-gray-200 focus:border-[#f45472] focus:ring-[#f45472]">
                        ...
                        <div>
                            ...
                            <input type="email" name="email" value="{{ old('email', $beautician->email) }}" 
                                   class="w-full px-4 py-3 text-sm rounded-2xl border-gray-200 focus:border-[#f45472] focus:ring-[#f45472]">
                        ...
                        {{-- Upload Foto Profil --}}
                        <div class="md:col-span-2">
                            <label class="block text-xs font-bold uppercase tracking-wider text-gray-700 mb-2">Foto Profil Staf</label>

                            @if($beautician->photo)
                                <div class="mb-3 flex items-center gap-4 p-3 bg-gray-50 rounded-2xl border border-gray-100">
                                    <img src="{{ $beautician->photo_url }}" alt="Preview" class="w-14 h-14 rounded-full object-cover border border-rose-200">
                                    <div>
                                        <p class="text-xs font-bold text-gray-800">Foto Saat Ini</p>
                                        <p class="text-[10px] text-gray-400 font-mono">{{ $beautician->photo }}</p>
                                    </div>
                                </div>
                            @endif
                            ...
                            <p class="text-[10px] text-gray-400 mt-1">Kosongkan jika tidak ingin mengubah foto profil.</p>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="bio" rows="3" required 
                                      class="w-full px-4 py-3 text-sm rounded-2xl border-gray-200 focus:border-[#f45472] focus:ring-[#f45472]">{{ old('bio', $beautician->bio) }}</textarea>
                            @error('bio') <p class="text-xs text-rose-500 mt-1">{{ $message }}</p> @enderror
                        </div>

                        {{-- Status Aktif --}}
                        <div class="md:col-span-2 flex items-center gap-3 pt-2">
                            <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $beautician->is_active) ? 'checked' : '' }} 
