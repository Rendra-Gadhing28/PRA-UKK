...
### **11.2 Security**
```yaml
NFR-7: CSRF protection (built-in Laravel)
NFR-8: XSS prevention (Blade auto-escape)
NFR-9: SQL injection prevention (Eloquent)
NFR-10: Input validation (server + client)
NFR-11: File upload validation (size, type)
NFR-12: Rate limiting (optional)
NFR-13: Authentication required for protected routes
NFR-14: Admin middleware for admin routes
NFR-15: Password hashing (bcrypt)
```

### **11.3 Usability**
```yaml
NFR-16: Mobile-responsive (Tailwind responsive classes)
NFR-17: Intuitive navigation (max 3 clicks to book)
NFR-18: Clear error messages (Bahasa Indonesia)
NFR-19: Loading states (spinner, skeleton)
NFR-20: Visual feedback (toast notifications)
NFR-21: Confirmation dialogs for destructive actions
```

### **11.4 Reliability**
```yaml
NFR-22: Race condition prevention (3-layer defense)
NFR-23: Unique constraints di database
NFR-24: Graceful error handling
NFR-25: Session management
NFR-26: Cache invalidation strategy
```
